...
6. If active and the message has no image, it's checked against a small set of text commands (all in [commands.py](commands.py)) before falling back to the clarification-resume flow: `"help"`/`"menu"` (`HELP_TEXT`, the full command list), `"today"`/`"progress"` (`build_today_summary` — lists today's logged meals plus the same progress line as step 9), `"undo"` (`undo_last_meal` — deletes the `NutritionEstimateRecord` for the most recent meal logged *today*, marks the `Meal` `"undone"`; the row stays for audit but drops out of every aggregation query), and `"reminders off"` / `"reminders on"` (toggles `User.reminders_enabled`, handled inline in `main.py`). Anything else falls through to `get_pending_meal` ([commands.py](commands.py)), which looks for the user's most recent `Meal` still waiting on a clarification (`needs_new_image` / `awaiting_dish_choice` / `awaiting_serving_size`). If one exists, the reply text is stored on `Meal.clarification_text`, the meal is closed out as `clarification_closed`, and the user is asked to resend a photo — we don't fabricate a nutrition estimate from text alone, consistent with the "don't trust it, don't log it" policy below. Otherwise the user is told to send an image (and pointed at `"help"`).
7. The background task, `analyze_and_reply` in [analysis_job.py](analysis_job.py), opens its own DB session (background tasks run after the request-scoped session is closed), calls `analyze_meal_image` ([nutrition.py](nutrition.py)) with up to 3 attempts (short backoff) to absorb transient Gemini failures, sending the image to Gemini (`gemini-flash-latest`) and validating the JSON response against the `MealImageAnalysis` Pydantic schema.
...
9. On auto-log, a `NutritionEstimateRecord` row is persisted with min/max ranges for calories/protein/carbs/fat/fibre, and the reply includes today's cumulative progress against the user's goals (`get_daily_nutrition_totals` in [daily_progress.py](daily_progress.py), which sums estimates within the user's local calendar day using `Asia/Kolkata`, converting to naive UTC via `local_day_bounds_utc` to match how SQLite stores timestamps — also reused by `commands.build_today_summary` and `commands.undo_last_meal`). `format_daily_progress` expresses progress as a remaining amount (`format_remaining`, e.g. `"1,050–1,300 kcal left"`) rather than a percentage, since two overlapping percentage ranges was the least intuitive part of the old copy.
...
At startup, `recover_stuck_meals` in [analysis_job.py](analysis_job.py) sweeps any `Meal` still at `status="stored"` — these are meals whose analysis `BackgroundTask` never finished (e.g. the process restarted mid-analysis) and would otherwise sit silently unprocessed forever. Each is marked `analysis_failed` and the user is notified to resend the photo.

`main.py` itself is now just app/lifespan wiring, Twilio signature validation, and the `/whatsapp` route's dispatch logic — the pieces it dispatches to (`commands.py`, `analysis_job.py`, `daily_progress.py`) live in their own modules.

Data model ([models.py](models.py)): `User` (keyed by normalized phone, holds goals + onboarding state) → `Meal` (one per inbound image, keyed by Twilio `MessageSid` for idempotency) → `NutritionEstimateRecord` (one-to-one, the accepted result — named to avoid colliding with the Pydantic `NutritionEstimate` schema in [nutrition.py](nutrition.py)) and `MealAnalysisAttempt` (one-to-many, every analysis/decision attempt including failures, for debugging the model and the policy). `InboundMessage` (keyed by `MessageSid`) and `OutboundMessage` form the message audit trail described above.
